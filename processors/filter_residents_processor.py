...
def process_filter_residents_file(
    ...
    try:
        ...
        required_res_cols = ["Client_ID_Number", "Last_Name", "First_Name", "Middle_Name"]
        df_residents = pd.read_excel(residents_file_path, usecols=required_res_cols)
        ...
        if duplicated_ids:
            ...
            log_fn("We suggest resolving this before proceeding.")

        orig_cols = df_import.columns.tolist()

        if "Client_ID_Number" not in df_import.columns:
            log_fn("⚠️ Import file does not contain Client_ID_Number.")
            if "Resident" not in df_import.columns:
                log_fn("⚠️ Import file also does not contain Resident. Cannot match residents.")
                return None

            df_import["Client_ID_Number"] = pd.NA

        mask = df_import["Client_ID_Number"].isna() | (
            df_import["Client_ID_Number"].astype(str).str.strip() == ""
        )
        ...
        if all_empty_or_nan:
            ...
            if "Resident" not in df_import.columns:
                log_fn("⚠️ No Resident column found. Cannot match by name.")
                return None

            df_import = df_import.drop(columns=["Client_ID_Number"], errors="ignore")

            resident_series = df_import["Resident"].fillna("").astype(str)
            ...
            if resident_series.str.contains(",", na=False).any():
                df_import["Resident"] = resident_series.str.upper().str.strip()
            ...
            residents_long = (
                ...
                .assign(Resident_variants=lambda df: df[["Resident_1", "Resident_2"]].values.tolist())
            ...
            df_import_filtered = (
                df_import
                .merge(df_residents[["Client_ID_Number"]], on="Client_ID_Number", how="inner")
                .reset_index(drop=True)
            )

        if df_import_filtered.empty:
            log_fn("⚠️ No matching residents found.")
            log_fn("Please make sure this is the right residents file and that Client_ID_Number or Resident names match.")
            return None
